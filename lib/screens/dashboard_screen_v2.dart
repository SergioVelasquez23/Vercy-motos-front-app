...
class _DashboardScreenV2State extends State<DashboardScreenV2>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      debugPrint('🔑 [Dashboard] roles=${userProvider.roles} '
          'isAdmin=${userProvider.isAdmin} isSuperAdmin=${userProvider.isSuperAdmin} '
          'isAsesor=${userProvider.isAsesor}');
      ...
      if (userProvider.isAdmin) {
        ...
        // El dashboard no tiene contenido para asesores (todo está reservado a
        // admin/superadmin), así que se redirige a su panel real para evitar
        // que vean una pantalla en blanco al llegar aquí (deep link o acceso
        // directo de iPhone guardado apuntando a /dashboard).
        setState(() { _isLoading = false; });
        if (mounted) context.go('/asesor-pedidos');
      } else {
  ...
  Future<void> _cargarEstadisticas({bool forceRefresh = false}) async {
    try {
      final dashboardData = await _reportesService.getDashboard(
        ...
        soloElectronicos: true,
      );

      if (mounted && dashboardData != null) {
        setState(() {
          _dashboardData = dashboardData;
          if (_objetivosTemporales.isNotEmpty) _objetivosTemporales.clear();
        });
