...
class ItemPedidoTestSuite {
  ...
  static void _testCalculoSubtotal() {
    ...
    for (final test in testCases) {
      try {
        ...
        assert((subtotalCalculado - subtotalEsperado).abs() < 0.01,
               'Subtotal incorrecto: esperado $subtotalEsperado, obtenido $subtotalCalculado');
        
        print('  ✅ ${test['cantidad']}x ${test['precio']} = $subtotalCalculado');
      } catch (e) {
  ...
  static void _testCompatibilidadBackend() {
    ...
    for (int i = 0; i < formatosBackend.length; i++) {
      try {
        ...
        assert((item.subtotal - subtotalEsperado).abs() < 0.01,
  ...
  static void _testValidaciones() {
    ...
    for (int i = 0; i < casosInvalidos.length; i++) {
      try {
        ...
        print('  ✅ Validación negativa ${i+1} - OK (${e.toString().split(':').last.trim()})');
      }
    }
  }
  ...
  static void _testRoundTripCompatibility() {
    ...
    try {
      ...
      jsonBackend['subtotal'] = jsonBackend['cantidad'] * jsonBackend['precioUnitario'];
      ...
      assert(itemOriginal.ingredientesSeleccionados.length == itemReconstruido.ingredientesSeleccionados.length);
